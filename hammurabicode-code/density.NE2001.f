...
c--CHANGE-Added extra variable, sungc.
...
c need to initiate values in case components are flagged out
        ...
        if(wg1 .eq. 1) ne1 = ne_outer(x,y,z, F1,sungc)
        if(wg2 .eq. 1) ne2 = ne_inner(x,y,z, F2)
        if(wga .eq. 1) nea = ne_arms_log_mod(x,y,z,whicharm,Fa,sungc)
c-----Potential ALT--------------
c-------------------------------------------------------------------------------------------------
c if the line below is commented out, the galactic center component will be ignored (23/06 AD 2006)
        if(wggc .eq. 1) negc = ne_gc(x,y,z, Fgc)
c-------------------------------------------------------------------------------------------------
c-------------------------------------------------------------------------------------------------
c if the following lines are taken out, the lism components are neglected
c usefull to generate a magfield based on the ne distribution, without the annoyance of LISM ne variations
c
        if(wglism .eq. 1)
     .     nelism = ne_lism(x,y,z,Flism,wlism,wldr,wlhb,wlsb,wloopI)
        if(wgcN .eq. 1) call neclumpN(x,y,z,necN,FcN,hitclump,sungc)
        if(wgvN .eq. 1) call nevoidN(x,y,z,nevN,FvN,hitvoid,wvoid,sungc)
c--------------------------------------------------------------------------------------------------
...
c--CHANGE added sungc
        SUBROUTINE GET_PARAMETERS(sungc)
c-----------------------------------------------------------------------
        implicit none
c--CHANGE substitute rsun by sungc------------------------------
c       real rsun
c       common /mw/ rsun
c       data rsun/8.5/
c--CHANGE Added extra variable sungc--------------------------
...
c    .           farm(1), farm(2), farm(3), farm(4), farm(5)
        ...
        REAL FUNCTION NE_ARMS_LOG_MOD(x,y,z, whicharm, Farms, sungc)
c-----------------------------------------------------------------------
...
50        continue
        ...
        REAL FUNCTION NE_OUTER(x,y,z, F_outer, sungc)
c-----------------------------------------------------------------------
